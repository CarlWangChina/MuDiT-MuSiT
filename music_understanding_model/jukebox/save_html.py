import os
import json
from Code_for_Experiment.Metrics.music_understanding_model.tensorboardX.tests.test_numpy import np
...
def save_html(logdir, x, zs, labels, alignments, hps):
    ...
    with open(f'{logdir}/index.html', 'w') as html:
        print(f"<html><head><title>{logdir}</title></head><body style='font-family: sans-serif; font-size: 1.4em; font-weight: bold; text-align: center; max-width:1024px; width: 100%; margin: auto;'>", file=html)
        print("<link rel='icon' href='data:;base64,iVBORw0KGgo='>", file=html)
        for item in range(bs):
            data = dict(wav=x[item].cpu().numpy(), sr=hps.sr, info=labels['info'][item], total_length=total_length, total_tokens=len(labels['info'][item]['full_tokens']), alignment=alignments[item] if alignments is not None else None)
...
def _save_item_html(item_dir, item_id, item_name, data):
    ...
    with open(f'{item_dir}/index.html', 'w') as html:
        print(f"<html><head><title>{item_name}</title></head><body style='font-family: sans-serif; font-size: 1.4em; font-weight: bold; text-align: center; max-width:1024px; width: 100%; margin: auto;'>", file=html)
        print("<link rel='icon' href='data:;base64,iVBORw0KGgo='>", file=html)
