...
def load_audio(file, sr, offset, duration, resample=True, approx=False, time_base='samples', check_duration=True):
    ...
    offset = int(offset / sr / float(audio.time_base))
...
def test_dataset_loader():
    ...
    train_loader = DataLoader(dataset, batch_size=hps.bs, num_workers=hps.nworkers, pin_memory=False, sampler=sampler, drop_last=True, collate_fn=collate_fn)
    dist.barrier()
    sampler.set_epoch(0)
