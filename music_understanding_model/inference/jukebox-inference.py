...
host = {"dist_backend": "nccl", "master_addr": "127.0.0.1", "master_port": 25681}
...
def process(rank:int, world_size:int, host:dict, data_loader_num_workers:int=2):
    ...
    device = f"cuda:{rank}"
    ds = dataset.AudioDataset(pathes=pathes, rank=rank, rootPath=input_dir, statePath=state_dir, outputPath=output_dir, getLength=True)
