...
def normalize_loudness(audio: torch.Tensor, sampling_rate: int, target_loudness: float) -> torch.Tensor:
    ...
    if audio.dim() == 3:
        audio_list = [
            pyln.normalize.loudness(audio[i].numpy(), pyln.Meter(sampling_rate).integrated_loudness(audio[i].numpy()), target_loudness)
            for i in range(audio.shape[0])
        ]
        return torch.stack([torch.from_numpy(x) for x in audio_list], dim=0)
