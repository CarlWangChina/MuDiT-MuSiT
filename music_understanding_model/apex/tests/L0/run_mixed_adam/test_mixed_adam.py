...
class TestFusedAdam(unittest.TestCase):
    ...
    def gen_single_type_test(self, param_type=torch.float):
        nelem = 278011
        adam_option = {'lr':5e-4, 'betas':(0.9, 0.999), 'eps':1e-08, 'weight_decay':0, 'amsgrad':False}
    ...
    def test_multi_params(self):
        ...
        ref_param, tst_param, ref_optim, tst_optim = self.gen_param_optim(tensors, adam_option)
        for i in range(self.iters):
            half_grads = self.gen_mixed_grad(ref_param, tst_param)
            ref_optim.step()
            tst_optim.step(grads=half_grads)
    ...
    def test_adam_option(self):
        nelem = 1
        adam_option = {'lr':0.01, 'betas':(0.6, 0.9), 'eps':3e-06, 'weight_decay':0, 'amsgrad':False}
        tensor = torch.rand(nelem, dtype=torch.float, device='cuda')
        ref_param, tst_param, ref_optim, tst_optim = self.gen_param_optim([tensor], adam_option)
        for i in range(self.iters):
            self.gen_grad(ref_param, tst_param)
