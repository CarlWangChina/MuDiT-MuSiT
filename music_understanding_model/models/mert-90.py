...
class mert90Processor:
    def __init__(
        ...
        cache_dir="/home/dev-contributor/cache/",
    ...
    def get_mert_features(self, waveform: torch.Tensor, output_file_name: str):
        ...
        with torch.no_grad():
            with (gzip.open(output_file_name, "wb") as output_file):
                for i in range(0, len(waveform), window_size_in_samples):
                    ...
                    feature = feature["last_hidden_state"][0, :data_size_in_window, :].to("cpu").type(torch.float32).flatten()
                    output_file.write(feature.numpy().tobytes())
                    del feature

    def process(self, dataArray, sample_rate, outFile):
