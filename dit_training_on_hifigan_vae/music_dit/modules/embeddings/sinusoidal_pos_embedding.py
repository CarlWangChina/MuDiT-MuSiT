...
def _get_embedding(num_embeddings: int, embedding_dim: int, scaling_factor: float = 10000.0) -> torch.Tensor:
    ...
    x = (torch.arange(num_embeddings, dtype=torch.float32).reshape(-1, 1) /
         torch.pow(scaling_factor, torch.arange(0, embedding_dim, 2, dtype=torch.float32) / embedding_dim))
