...
class DiT(nn.Module):
    ...
    def forward_with_cfg(self,
        ...
        if self.use_learned_variance:
            out_cond, variance_cond = self.forward(x,
                                                   timesteps=timesteps,
                                                   prompt=prompt,
                                                   positions=positions,
                                                   condition=condition,
                                                   ...
                                                   context_positions=context_positions,
                                                   context_mask=context_mask)
            out_uncond, variance_uncond = self.forward(x,
                                                       timesteps=timesteps,
                                                       prompt=prompt,
                                                       positions=positions,
                                                       padding_mask=padding_mask)
            ...
            out_uncond = self.forward(x,
                                      ...
                                      positions=positions,
                                      padding_mask=padding_mask)
            variance_cond = variance_uncond = None
        out = out_uncond + cfg_scale * (out_cond - out_uncond)
        if self.use_learned_variance:
            variance = variance_uncond + cfg_scale * (variance_cond - variance_uncond)
            return out, variance
        else:
            return out