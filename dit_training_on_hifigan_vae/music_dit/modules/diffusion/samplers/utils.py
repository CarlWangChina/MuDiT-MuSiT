...
def update_time_steps(num_training_steps: int, num_inference_steps: int, timestep_spacing: str = "leading", steps_offset: int = 0) -> torch.Tensor:
    assert num_inference_steps <= num_training_steps, (f"The number of inference steps ({num_inference_steps}) should be less than " f"the number of training steps ({num_training_steps}).")
