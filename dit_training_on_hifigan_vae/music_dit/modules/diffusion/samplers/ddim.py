...
from .utils import make_beta_schedule, update_time_steps, threshold_sample
...
class DDIMSampler(nn.Module):
    ...
    def denoise(self,
        ...
        sqrt_beta_cumprod = self.sqrt_betas_cumprod[current_timestep]
        sqrt_alpha_cumprod = self.sqrt_alphas_cumprod[current_timestep]
        alpha_cumprod_prev = self.alphas_cumprod[prev_timestep] if prev_timestep is not None else 1.0
        if isinstance(model_output, tuple):
            model_output, log_var = model_output
            min_log_variance = self.posterior_log_variance_clipped[current_timestep]
            max_log_variance = torch.log(self.betas[current_timestep])
            log_var = 0.5 * (log_var + 1) * (max_log_variance - min_log_variance) + min_log_variance
            variance = torch.exp(log_var)
        else:
            variance = self.posterior_variance
        pred_original_sample = (sample - sqrt_beta_cumprod * model_output) / sqrt_alpha_cumprod
        ...
        if use_clipped_model_output:
            pred_epsilon = (sample - sqrt_alpha_cumprod * pred_original_sample) / sqrt_beta_cumprod
        pred_sample_direction = ((1 - alpha_cumprod_prev - sigma**2) ** 0.5) * pred_epsilon
        prev_sample = (alpha_cumprod_prev ** 0.5) * pred_original_sample + pred_sample_direction
        if eta > 0:
