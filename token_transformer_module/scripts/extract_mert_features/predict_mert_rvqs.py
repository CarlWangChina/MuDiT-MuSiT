...
if __name__ == "__main__":
    logger.info("Predicting RVQ tokens ...")
    CONFIG_FILE = "config.yaml"
    parallel_enabled = torch.cuda.is_available()
    _configs = OmegaConf.load(current_path / CONFIG_FILE)
    _configs = _configs.rvq_predictor
    _configs.root_path = str(root_path)
    _configs.current_path = str(current_path)
    start_parallel_processing(processor_class_name="ama-prof-divi.pre_processing.mert_extractor.RVQPredictor",
