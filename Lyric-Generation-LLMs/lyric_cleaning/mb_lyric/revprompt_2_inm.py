...
_cnxp = mysql.connector.pooling.MySQLConnectionPool(
    ...
    **_dbconfig)
_st_qry = _st_iou = _st_uo0 = _st_uo1 = _st_uo2 = None

def _ensure_sk() -> PooledMySQLConnection:
    cnx = _cnxp.get_connection()
    cursor = cnx.cursor()
    cursor.execute(f"SELECT key FROM oai_key_usage WHERE rid = '{_rid}'")
    skr = cursor.fetchone()
...
def _core_job(sdict, sio: TIOW, eio: TIOW, stst: list[int], lock):
    ...
    if not lur or len(lur) < 48:
        _logger.warning('Empty or too short lrc source, Should '
                        f'check column `lrc_text` of id({sid})!')
        if cnx := _try_get_cnx(sid):
            try:
                udt = time.time()
                ecv = '-E0'
                cursor = cnx.cursor()
                cursor.execute(_st_iou, (sid, udt, ecv, ecv, udt,))
                cnx.commit()
                cursor.close()
                cnx.close()
            except:
