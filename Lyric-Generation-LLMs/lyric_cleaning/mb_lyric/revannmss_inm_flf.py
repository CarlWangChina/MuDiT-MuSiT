...
from llmrelate import oai_px_skey, pxg_req_wu, spt_rl2ams
...
bjfts = bj_dt.strftime('%y%m%d%H%M%S')
...
logging.basicConfig(
    format='%(asctime)s|%(levelname)s|'
           '%(process)d-%(thread)d|'
           '%(name)s: %(message)s',
    filename=f"{_lfdir}/rc_{bjfts}.log",
    encoding='utf-8',
    level=logging.INFO)
...
_rid = sys.argv[1]
_st_iou = _st_uo0 = _st_uo1 = _st_uo2 = None

def _ensure_sk() -> PooledMySQLConnection:
    cnx = _cnxp.get_connection()
    cursor = cnx.cursor()
    cursor.execute(f"SELECT key,round_id FROM oai_key_usage WHERE round_id=%s", (_rid,))
    skr = cursor.fetchone()
    if not skr: raise Exception('Should insert a new round row to table '
                                 '`oai_key_usage` or check the 3rd param from passed in!')
    if skr[0] != oai_px_skey: raise Exception(
        'You must be forget to change the `OAI_PX_API_KEY` in .env!')
...
def _ud_oku_st(cnx: PooledMySQLConnection):
    ...
    cursor = cnx.cursor()
    cursor.execute(_st_uo0, (sts, _rid,))
    cnx.commit()
    cursor.close()
    cnx.close()
...
def _ud_oku_ct():
    ...
    cnx = _cnxp.get_connection()
    cursor = cnx.cursor()
    cursor.execute(_st_uo2, (cts, _rid,))
    cnx.commit()
    cursor.close()
    cnx.close()
...
def _load_songs() -> list[dict[str, str]]:
    cnx = _ensure_sk()
    with open('./data/ouput/inm_l2s_rc/songs.json', 'r', encoding='utf-8') as f:
        songs = json.load(f)
    _logger.info(f'total {len(songs)} songs queried.')
    _ud_oku_st(cnx)
    return songs
...
def _core_dbw(sid, rpc, usage, sio: TIOW, eio: TIOW, stst: list, lock):
    ...
    try:
        ...
        udt = time.time();
        p, c, t = usage
        cursor.execute(_st_uo1, (p, c, t, udt, _rid,))
        if rpc is None:
            cnx.commit()
            cursor.close()
            cnx.close()
            _logger.error(f'SID: {sid}, LLM RETURNED None')
            _wf_ladd(lock, stst, 0, eio, {'id': sid, 'er': 'NONE rpc'})
            return
        if len(rpc) > 4095:
            _logger.warning(f'Too long rpc of sid{sid}, will be cut off '
                            'when saving to db, must check it manually.')
        _wf_ladd(lock, stst, 1, sio, {'id': sid, 'rpc': rpc})
        cursor.execute(_st_iou, (sid, udt, rpc, rpc, udt,))
        cnx.commit()
        cursor.close()
        cnx.close()
    except:
        ...
        errord = {'id': sid, 'er': 'DB-CMT ERR',
                  'rpc': rpc, 'usg': list(usage)}
    if errord:
        _wf_ladd(lock, stst, 0, eio, errord)
...
def _core_job(sdict, sio: TIOW, eio: TIOW, stst: list[int], lock):
    ...
    lur = re.sub(_regft, '', sdict['lur']).strip()
    preclrc = _msmuscle.wash_lrc_4ams(lur)
    if preclrc.count('\n') < 5:
        _logger.warning(f'Lines < 6 after pre-clean of id({sid})!')
        if cnx := _try_get_cnx(sid):
            try:
                cursor = cnx.cursor()
                t = time.time()
                cursor.execute(_st_iou, (sid, t, '-E0', '-E0', t,))
                cnx.commit()
                cursor.close()
                cnx.close()
            except:
    ...
    usage = None
    try:
        rpc, usage = pxg_req_wu(spt_rl2ams, preclrc)
...
def _mthp_start(pi: int, songs: list[dict[str, str]]):
    ...
    sfp = f'{_dodir}/r{_rid}_sp{pi}.jsonl'
    efp = f'{_dodir}/r{_rid}_sp{pi}_err.jsonl'
...
if __name__ == "__main__":
    ...
    _logger.info(f'try to use {ucc} cpu-cores.')
    _mpmt_start(ucc)
    print("Nothing bad happen for running ps, check detail log.")
    _logger.info(f'has done multiprocess jobs.')

global _st_iou, _st_uo0, _st_uo1, _st_uo2
_st_iou = "INSERT INTO song_oai_result(song_id,update_time,response,response_clean,request_time) VALUES (%s,%s,%s,%s,%s)"
_st_uo0 = "UPDATE oai_key_usage SET start_time=%s WHERE round_id=%s"
_st_uo1 = "INSERT INTO song_oai_request(song_id,prompt_count,char_count,token_count,request_time,round_id) VALUES (%s,%s,%s,%s,%s,%s)"
_st_uo2 = "UPDATE oai_key_usage SET complete_time=%s WHERE round_id=%s"