...
_cnxp = mysql.connector.pooling.MySQLConnectionPool(
    ...
    **_dbconfig)
_st_qry = _st_uil = _st_uo0 = _st_uo1 = _st_uo2 = None

def _ensure_sk() -> PooledMySQLConnection:
    cnx = _cnxp.get_connection()
    cursor = cnx.cursor()
    cursor.execute(f"SELECT key, round_id FROM oai_key_usage WHERE round_id = '{_rid}'")
    skr = cursor.fetchone()
...
def _core_dbw(sid, rpc, usage, sio: TIOW, eio: TIOW, stst: list, lock):
    ...
    try:
        ...
        if rpc is None:
            ...
            return
        if len(rpc) > 4095:
            _logger.warning(f'Too long rpc of sid{sid}, will be cut off when saving to db, must check it manually.')
        _wf_ladd(lock, stst, 1, sio, {'id': sid, 'rpc': rpc})
        cursor.execute(_st_uil, (rpc, udt, sid,))
        cnx.commit()
        cursor.close()
        cnx.close()
    except:
...
def _core_job(sdict, sio: TIOW, eio: TIOW, stst: list[int], lock):
    ...
    if preclrc.count('\n') < 5:
        ...
        if cnx := _try_get_cnx(sid):
            try:
                cursor = cnx.cursor()
                cursor.execute(_st_uil, ('-E0', time.time(), sid,))
                cnx.commit()
                cursor.close()
                cnx.close()
            except:
