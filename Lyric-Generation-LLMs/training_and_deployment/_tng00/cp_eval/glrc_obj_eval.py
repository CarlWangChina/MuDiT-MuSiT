...
def find_msnt_indices(text: str):
    """Return list of tuples, with matched start and end index for each."""
    matches = re.finditer(r'\([\w-]+\)\n?', text)
    indices = [(m.start(), m.end()) for m in matches]
    if not indices or indices[0][0] != 0:
...
def cnv_lrc_2cmp_infos(llm_gen: str):
    ...
    msnt_indices = find_msnt_indices(llm_gen)
...
def cnv_mss_2cmp_infos(msstr: str):
    ...
    msnt_indices = find_msnt_indices(msstr)
    mindices_len = len(msnt_indices)
    rt_ttlc = 0
    rt_sidl = []
    rt_sncs = ''
