...
class VAE2Model(nn.Module):
    def __init__(self, training: bool = False):
        ...
        self.encoder = SEANetEncoder(num_channels=hparams.num_channels,
                                     ...
                                     final_activation=hparams.final_activation_encoder,
        ...
        self.decoder = SEANetDecoder(num_channels=hparams.num_channels,
                                     ...
                                     activation=hparams.activation,
                                     activation_params=hparams.activation_params,
                                     final_activation=hparams.final_activation_decoder,
                                     norm=hparams.norm_type,
                                     norm_params=hparams.norm_params,
                                     dropout=hparams.dropout_rate)
    ...
    def encode(self, audio: torch.Tensor, padding_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    @staticmethod
    def _reparameterize(mean: torch.Tensor, log_var: torch.Tensor, eps: Optional[torch.Tensor] = None, rank: int = 0, world_size: int = 1) -> torch.Tensor:
        if eps is None:
    ...
    def forward(self, audio_frame: torch.Tensor, eps: Optional[torch.Tensor] = None, rank: int = 0, world_size: int = 1) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
