...
class DiscriminatorP(nn.Module):
    def __init__(self, channels: int, period: int, kernel_size: int = 5, stride: int = 3, use_spectral_norm: bool = False):
        super(DiscriminatorP, self).__init__()
        self.period = period
        norm_f = weight_norm if use_spectral_norm == False else spectral_norm
        self.convs = nn.ModuleList([
            norm_f(nn.Conv2d(channels, 32, (kernel_size, 1), (stride, 1), padding=(get_padding(kernel_size, 1), 0))),
            norm_f(nn.Conv2d(32, 64, (kernel_size, 1), (stride, 1), padding=(get_padding(kernel_size, 1), 0))),
            norm_f(nn.Conv2d(64, 256, (kernel_size, 1), (stride, 1), padding=(get_padding(kernel_size, 1), 0))),
            norm_f(nn.Conv2d(256, 512, (kernel_size, 1), (stride, 1), padding=(get_padding(kernel_size, 1), 0))),
            norm_f(nn.Conv2d(512, 512, (kernel_size, 1), 1, padding=(2, 0))),
        ])
...
class DiscriminatorS(nn.Module):
    def __init__(self, channels: int, use_spectral_norm: bool = False):
        ...
        norm_f = weight_norm if use_spectral_norm == False else spectral_norm
        self.convs = nn.ModuleList([
            norm_f(nn.Conv1d(channels, 64, 7, 1, padding=3)),
            norm_f(nn.Conv1d(64, 64, 21, 2, groups=4, padding=10)),
            norm_f(nn.Conv1d(64, 128, 21, 2, groups=8, padding=10)),
            norm_f(nn.Conv1d(128, 256, 21, 4, groups=8, padding=10)),
            norm_f(nn.Conv1d(256, 512, 21, 4, groups=8, padding=10)),
            norm_f(nn.Conv1d(512, 512, 21, 1, groups=8, padding=10)),
            norm_f(nn.Conv1d(512, 512, 3, 1, padding=1)),
        ])
