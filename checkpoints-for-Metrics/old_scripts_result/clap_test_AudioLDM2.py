...
if __name__ == '__main__':
    ...
    res_clap = []
    for i in trange(1, 401):
        path = '/home/zhehui/FAD/AudioLDM2/' + str(i) + '.wav'
        ...
        for _ in range(1, repeat_num):
            res2 += clap_test([path], [prompt1, prompt2])
        res2 /= repeat_num
        res_clap.append((path, res2[0, 0].item(), res2[0, 1].item()))
    with open("/home/zhehui/wangqi/result_AudioLDM2.csv","w",encoding="utf-8") as f:
        writer = csv.writer(f)
        for i in range(400):
            writer.writerow(res_clap[i])

    '''
    gt_file_list = ['D:/wangqi/2.mp3','D:/wangqi/4.mp3']
    audio_file_list = ['D:/wangqi/1.mp3','D:/wangqi/3.mp3']
    
    text_list = ["流行歌曲, 抒情, 百年孤独, 寂寞"]
    res = mert_test(gt_file_list, audio_file_list)
    res2 = clap_test(['D:/wangqi/1.mp3', 'D:/wangqi/2.mp3'], ["流行歌曲, 抒情, 百年孤独, 寂寞", "流行歌曲, 抒情, 百年孤独, 寂寞","摇滚歌曲，激烈，喝酒，伤感"])
    print(res)
    print(res2)
    '''
