...
class MultiHeadAttention(nn.Module):
    def __init__(self, input_dim: int, num_heads: int, hidden_dim: Optional[int] = None, context_dim: Optional[int] = None, qkv_bias: bool = False, rope: Optional[RotaryPosEmbedding] = None, use_rpr: bool = False, max_position: int = 10000, attn_dropout: float = 0.0, proj_dropout: float = 0.0):
    ...
    def forward(self, x: torch.Tensor, *, positions: Optional[torch.Tensor] = None, padding_mask: Optional[torch.Tensor] = None, context: Optional[torch.Tensor] = None, context_positions: Optional[torch.Tensor] = None, context_mask: Optional[torch.Tensor] = None, causal_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.rope is not None:
            ...
            assert positions.size() == (x.size(0), x.size(1)), (f"Position tensor {positions.size()} must have the same shape as input {(x.size(0), x.size(1))}.")
            assert context_positions is not None, "Context position tensor must be provided when RoPE is used."
            assert context_positions.size() == (context.size(0), context.size(1)), (f"Context position tensor {context_positions.size()} must have the same shape as context {(context.size(0), context.size(1))}.")
