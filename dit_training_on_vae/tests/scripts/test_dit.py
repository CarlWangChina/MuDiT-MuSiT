...
class TestDiT(unittest.TestCase):
    ...
    def test_dit_with_learned_sigma(self):
        ...
        dit = DiT(input_dim=64, hidden_dim=128, num_layers=4, num_heads=8, context_dim=32, use_cross_attention=True, device=self.device)
        ...
        y, variance = dit.forward_with_cfg(x, cfg_scale=0.6, timesteps=timestep, prompt=prompt, condition=condition, context=context)
        self.assertEqual(y.size(), (3, 128, 64))
        self.assertEqual(variance.size(), (3, 128, 64))

    def test_dit_without_learned_sigma(self):
        logger.info("Test DiT without leaned sigma")
        dit = DiT(input_dim=64, hidden_dim=128, num_layers=4, num_heads=8, context_dim=32, use_cross_attention=True, use_learned_variance=False, device=self.device)
        x = torch.randn(3, 128, 64).to(self.device)
        condition = torch.randn(3, 128, 128).to(self.device)
        context = torch.randn(3, 64, 32).to(self.device)
        timestep = torch.arange(3, device=self.device).long()
        ...
        self.assertEqual(y.size(), (3, 128, 64))
        y = dit.forward_with_cfg(x, cfg_scale=0.6, timesteps=timestep, condition=condition, context=context)
        self.assertEqual(y.size(), (3, 128, 64))