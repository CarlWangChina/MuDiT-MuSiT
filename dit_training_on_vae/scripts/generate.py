...
DEFAULT_MEL_MEAN = None
DEFAULT_MEL_STD = None
...
def main():
    ...
    logger.info("Building the Clap model")
    clap_model = laion.clap.CLAP_Module(enable_fusion=False, amodel="HTSAT-base", tmodel="roberta")
    try:
    ...
    if args.reference is not None:
        ...
        length = ref_audio.size(-1)
        num_chunks = (length + CLAP_CHUNK_LENGTH - 1) // CLAP_CHUNK_LENGTH
    ...
    if args.lyrics is not None:
        ...
        try:
            ...
            phoneme_tokens = phoneme_tokenizer.tokenize(phoneme).unsqueeze(0)
            if (not args.cpu) and torch.cuda.is_available():
