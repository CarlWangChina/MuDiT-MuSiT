...
class TestLyrics2Phoneme(unittest.TestCase):
    def setUp(self):
        ...
        test_data_file = Path(__file__).parent.parent / "testdata" / "muchin_lyrics.txt"
        self.test_data = []
    ...
    def test_split_sentences(self):
        special_tokens = {'(verse)', '(chorus)', '(intro)', '(ending)', '(prechorus)', '(bridge)'}
        for i, lyrics in enumerate(self.test_data):
            sentences = self.lyrics2phoneme.split_sentence(lyrics, special_tokens=special_tokens)
