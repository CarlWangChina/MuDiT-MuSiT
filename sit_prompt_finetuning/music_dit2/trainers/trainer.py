...
class MusicDiTTrainer:
    ...
    def trim_datasets_(self,
                       dataset: DiTDataset) -> int:
        dist.barrier()
    ...
    def _get_data_from_batch(self,
                             batch,
                             dtype=torch.bfloat16) -> Dict[str, torch.Tensor]:
        return {
            "wet_vocal_vae": batch["wet_vocal_vae"]["data"].to(self.device).to(dtype).transpose(1, -1)[:, :self.max_vae_length, :],
            "wet_vocal_vae_mask": batch["wet_vocal_vae"]["padding_mask"].to(self.device).to(dtype)[:, :self.max_vae_length],
            "dry_vocal_vae": batch["dry_vocal_vae"]["data"].to(self.device).to(dtype).transpose(1, -1)[:, :self.max_vae_length, :] if "dry_vocal_vae" in batch else None,
            "dry_vocal_vae_mask": batch["dry_vocal_vae"]["padding_mask"].to(self.device).to(dtype)[:, :self.max_vae_length] if "dry_vocal_vae" in batch else None,
            "clap": batch["clap"]["data"].to(self.device).to(dtype).transpose(1, -1)[:, :self.max_clap_length, :] if "clap" in batch else None,
            "clap_mask": batch["clap"]["padding_mask"].to(self.device).to(dtype)[:, :self.max_clap_length] if "clap" in batch else None,
    ...
    def validate_loop(self,
        ...
        with (torch.no_grad()):
            for step, batch in enumerate(self.validation_dataloader):
                ...
                loss = self.model_engine(data["wet_vocal_vae"],
                                         data["wet_vocal_vae_mask"],
                                         data["dry_vocal_vae"],
                                         data["dry_vocal_vae_mask"],
                                         data["clap"],
                                         data["clap_mask"],
                                         data["lyrics"],
                                         data["lyrics_mask"],
                ...
                inference_loss = self.model_engine.test_inference(
                    wet_vocal_vae=data["wet_vocal_vae"],
                    wet_vocal_vae_mask=data["wet_vocal_vae_mask"],
                    dry_vocal_vae=data["dry_vocal_vae"],
                    dry_vocal_vae_mask=data["dry_vocal_vae_mask"],
                    clap=data["clap"],
    ...
    def train_step(self,
                   ...
                   epoch: int,
                   step_in_epoch: int,
                   epoch_length: int) -> Tuple[OrderedDict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        ...
        if self.config.log_lyrics_length:
            if data["lyrics"] is not None:
                ...
                self.logger.info("Rank %d, step %d, lyrics: %s", self.rank, current_step,
                                 "Dropped" if lyrics_dropout else "None")
        loss_dict = self.model_engine(data["wet_vocal_vae"],
                                      data["wet_vocal_vae_mask"],
                                      data["dry_vocal_vae"],
                                      data["dry_vocal_vae_mask"],
                                      data["clap"],
