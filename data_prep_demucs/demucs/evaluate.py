...
from . import distrib
from Code_for_Experiment.Targeted_Training.data_prep_demucs.demucs.utils import DummyPoolExecutor
...
def evaluate(solver, compute_sdr=False):
    ...
    with pool(args.test.workers) as pool:
        for index in indexes:
            ...
            estimates = apply_model(model, mix[None], shifts=args.test.shifts, split=args.test.split, overlap=args.test.overlap)[0]
            estimates = estimates * ref.std() + ref.mean()
            estimates = estimates.to(eval_device)
            references = th.stack([th.from_numpy(track.targets[name].audio).t() for name in model.sources])
            if references.dim() == 2:
            ...
            pendings.append((track.name, pool.submit(eval_track, references, estimates, win=win, hop=hop, compute_sdr=compute_sdr)))
        pendings = LogProgress(logger, pendings, updates=args.misc.num_prints, name='Eval (BSS)')
        tracks = {}
        ...
        for metric_name in metric_names:
            ...
            for source in model.sources:
                medians = [np.nanmedian(all_tracks[track][source][metric_name]) for track in all_tracks.keys()]
